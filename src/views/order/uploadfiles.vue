...
export default {
  ...
  methods: {
    ...
    async getgroupList() {
      ...
      const sysCode = "db";
      const { status, body } = await getgroupList({ sysCode, data, orderType: this.types });
      if (status == 200) {
        let { list } = body;
        // if( list.length){
        //   this.handRequired(list);
        // }
        console.log('zhe', this.types)
        list.length &&
          list.forEach((item) => {
            if (this.types == 68) {
            ...
            // 论文大礼包新增必传
            if (this.types == 65 && item.groupName == 'Readings') {
              item.required = true;
            }
            item.url = [];
            if (this.data.length) {
    ...
    formjudge() {
          ...
          if (this.type != 32) {
            ...
            if ((statuscounter1 + statuscounter2) == 2) {
              return status = true
				...
				// 新增Readings文件校验
        if (this.types == 65 && ['Readings'].includes(item.groupName) && !item.url?.length) {
          return status = false
        }
      });
      ...
      if (!status) {
        ...
        } else if (this.types == 65) {
          msgerror = "请上传Requirement/Readings文件";
        } else {
